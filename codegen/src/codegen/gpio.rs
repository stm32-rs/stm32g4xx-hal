...
fn gen_gpio_ip(ip: &gpio::Ip) -> Result<()> {
    ...
    // The STM32CubeMX database references a non-existing port I. This should be filtered out.
    let ports: Vec<_> = ports.into_iter().filter(|port| port.id >= 'A' && port.id <= 'G').collect();
...
fn get_port_pac_module(port: &Port, _feature: &str) -> String {
    format!("gpio{}", port.id.to_ascii_lowercase())
}
